...
int dma_inst::WR2TxBuffer()
{
	...
	for(i = 0; i < 446464; i ++) {
		TxPacket[i*4]   = i*4;
		TxPacket[i*4+1] = i*4+1;
		TxPacket[i*4+2] = i*4+2;
		TxPacket[i*4+3] = i*4+3;
	}
...
DmaToPL::DmaToPL(u32 DMA_DEV_ID_in,
		 u32 MAX_PKT_LEN_in,
		 u32 TX_BD_SPACE_HIGH_in,
		 u32 TX_BD_SPACE_BASE_in,
		 u32 NUMBER_OF_PACKETS_in,
...
int DmaToPL::TxSetup(XAxiDma * AxiDmaInstPtr)
{
	XAxiDma_BdRing *TxRingPtr;
	XAxiDma_Bd BdTemplate;
	int Delay = 0;
...
DmaFromPL::DmaFromPL(u32 DMA_DEV_ID_in,
		 u32 RX_BD_SPACE_HIGH_in,
		 u32 RX_BD_SPACE_BASE_in,
		 u32 MAX_PKT_LEN_RX_in,
		 u32 NUMBER_OF_PACKETS_RX_in,
...
int DmaFromPL::RxSetup(XAxiDma * AxiDmaInstPtr)
{
	XAxiDma_BdRing *RxRingPtr;
	int Delay = 0;
	int Coalesce = 1;
